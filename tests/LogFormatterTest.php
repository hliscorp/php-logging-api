...
class LogFormatterTest
{
    ...
    public function format()
    {
        ...
        $results[] = (new Strings($result))->assertEquals(date("Y-m-d H:i:s")." ".LOG_INFO." test 127.0.0.1 Chrome");
        
        $formatter = new LogFormatter("%d %v %e %f %l %m %u %i %a");
        $result = $formatter->format(new \Exception("testing"), LOG_EMERG);
        $results[] = (new Strings($result))->assertEquals(date("Y-m-d H:i:s")." ".LOG_EMERG." Exception ".__FILE__." ".(__LINE__-1)." testing test 127.0.0.1 Chrome");
